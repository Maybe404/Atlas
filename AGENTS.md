...
- Workspace role (`admin`/`editor`/`viewer`) — only `admin` manages members, spaces, trash, skills, audit.
...
**Shared contracts** (`packages/shared/src/index.ts`): Zod schemas + inferred domain types are the single source of truth shared by both apps. `fixtures.ts` is seed-only (not read at runtime). `html-metadata.ts` extracts title/summary from uploaded HTML.

**Frontend** (`apps/web/src/`, flat structure): React Query (`data-hooks.ts`) for all server state; `api-client.ts` is the only fetch layer (keeps `bun:sqlite` types out of the browser build). Uploaded HTML is rendered in a sandboxed iframe. URL routing via react-router. Note: most `.tsx` files carry `// @ts-nocheck` and are excluded from Biome (see `biome.json`).

## Conventions

- Biome: single quotes, semicolons, 2-space indent, 100-col width. `biome.json` excludes `apps/web/src/**/*.tsx`, `fixtures.ts`, migrations meta, and `*.html` — don't expect lint coverage there.
...
`todo.md` and `README.md` track current MVP status and known gaps (no email verification/SSO, no e2e, production CSP/cookie-secure hardening pending).
