...
const DEMO_LOGIN_ACCOUNTS = [
  { id: 'u1', email: 'lin@atlas.team', name: '林知远', initials: 'LZ', role: 'admin', joined: '2024-02', tint: 'var(--blue)' },
  { id: 'u2', email: 'chen@atlas.team', name: '陈夏', initials: 'CX', role: 'editor', joined: '2024-03', tint: '#ff9500' },
  { id: 'u5', email: 'he@atlas.team', name: '何远', initials: 'HE', role: 'viewer', joined: '2025-01', tint: '#34c759' },
];
...
export function canRead(doc, user) {
  ...
  if (doc.visibility === 'invite') return true;
...
function UserGlyph() {
  return (
    <svg width="15" height="15" viewBox="0 0 16 16" fill="none">
      ...
      <path d="M2.8 13.5c0.6-2.6 2.7-4.1 5.2-4.1s4.6 1.5 5.2 4.1" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" />
    </svg>
  );
}
...
function LogoutGlyph() {
  return (
    <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
      <path d="M6 2.5H4A1.5 1.5 0 0 0 2.5 4v6A1.5 1.5 0 0 0 4 11.5h2M8.5 4 11.5 7l-3 3M5 7h6.4" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
...
function SwitchGlyph() {
  return (
    <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
      <path d="M3 5h7l-1.6-1.6M11 9H4l1.6 1.6" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
...
function UserAvatar({ user, className = '', size }) {
  return (
    <span
      className={className}
      style={{ background: user?.tint || tintForUser(user), ...(size ? { width: size, height: size } : null) }}
    >
      {user?.initials}
    </span>
  );
...
export function UserMenu({ user, onLogin, onLogout, onSwitch }) {
  ...
  return (
    <div className={"user-menu-wrap " + (open ? 'open ' : '') + (user ? 'is-member' : 'is-guest')} ref={wrapRef}>
      {user ? (
        <button
        ...
        <div className="user-menu-pop" onClick={(event) => event.stopPropagation()}>
          {!user && (
            <div className="um-pane um-pane-guest">
              <div className="um-guest-head">
                <div className="um-guest-glyph"><UserGlyph /></div>
                <div className="um-guest-text">
                  <div className="um-name">游客身份</div>
                  <div className="um-sub">只能阅读公开的文章</div>
                </div>
              </div>
              <button className="um-cta" onClick={() => { close(); onLogin?.(); }}>
                登录账号
                <span className="um-cta-arrow">→</span>
              </button>
              <div className="um-hint">
                登录后可以访问<span className="um-hint-em">空间内邀请制文章</span>，并使用上传与设置功能。
              </div>
            </div>
          )}
          ...
          {user && pane === 'main' && (
            <div className="um-pane">
              <div className="um-head">
                ...
                <span className={"um-role role-" + user.role}>{ROLE_LABEL[user.role] || user.role}</span>
              </div>
              <div className="um-sep" />
              <button className="um-item" onClick={() => setPane('info')}>
                <span className="um-item-glyph"><InfoGlyph /></span>
                <span>查看信息</span>
                <span className="um-chev">›</span>
              </button>
              <button className="um-item" onClick={() => setPane('switch')}>
                <span className="um-item-glyph"><SwitchGlyph /></span>
                <span>切换账号</span>
                <span className="um-chev">›</span>
              </button>
              <div className="um-sep" />
              <button className="um-item um-item-danger" onClick={() => { close(); onLogout?.(); }}>
                <span className="um-item-glyph"><LogoutGlyph /></span>
                <span>退出登录</span>
              </button>
            </div>
          )}
          ...
          {user && pane === 'info' && (
            <div className="um-pane">
              <div className="um-pane-head">
                <button className="um-back" onClick={() => setPane('main')} aria-label="返回">‹</button>
              ...
              <dl className="um-info-grid">
                <dt>邮箱</dt><dd className="mono">{user.email}</dd>
                <dt>加入</dt><dd className="mono">{user.joined || '未记录'}</dd>
                <dt>账号 ID</dt><dd className="mono">{user.id}</dd>
                <dt>权限</dt><dd>{ROLE_LABEL[user.role] || user.role}</dd>
              </dl>
              <div className="um-sep" />
              <button className="um-item um-item-danger" onClick={() => { close(); onLogout?.(); }}>
                <span className="um-item-glyph"><LogoutGlyph /></span>
          ...
          {user && pane === 'switch' && (
            <div className="um-pane">
              ...
              <div className="um-switch-list">
                {DEMO_LOGIN_ACCOUNTS.map((account) => {
                  ...
                  return (
                    ...
                    >
                      <span className="um-switch-avatar" style={{ background: account.tint }}>{account.initials}</span>
              ...
              <div className="um-sep" />
              <button className="um-item" onClick={() => { close(); onLogin?.(); }}>
                <span className="um-item-glyph"><PlusGlyph /></span>
                <span>用其他邮箱登录</span>
              </button>
            </div>
          )}
      ...
      <div className="login-stage">
        <div className="login-brand">
          <div className="login-brand-mark">
            <svg width="44" height="44" viewBox="0 0 44 44" fill="none">
              <rect x="6" y="6" width="32" height="32" rx="3" stroke="currentColor" strokeWidth="1.6" />
              <path d="M14 28 L22 12 L30 28" stroke="currentColor" strokeWidth="1.6" strokeLinejoin="round" />
              <path d="M17.5 22 L26.5 22" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" />
            </svg>
          </div>
        ...
        <div className="login-panel">
          <div className="login-panel-inner">
            ...
            <p className="login-sub">输入团队邮箱与密码继续。第一次来？点击下方游客身份直接阅读公开文档。</p>
            ...
            <form className="login-form" onSubmit={submit}>
              ...
              <label className="login-field">
                <span className="login-label">
                  密码
                  <a className="login-forgot" onClick={(event) => event.preventDefault()} href="#">忘记密码？</a>
                </span>
                <input
                  type="password"
                  className="login-input"
              ...
              <button className={"login-submit " + (loading ? 'loading' : '')} type="submit" disabled={loading}>
            ...
            <div className="login-demo-list">
              {DEMO_LOGIN_ACCOUNTS.map((account) => (
                ...
                >
                  <span className="login-demo-avatar" style={{ background: account.tint }}>{account.initials}</span>
