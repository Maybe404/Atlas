...
//
// What the guard does: for a short window after an in-page anchor navigation, it keeps
// `scrollIntoView` from moving the ROOT scroller (it may still scroll inner containers,
// e.g. a TOC sidebar), which lets the anchor smooth scroll finish. Native behavior
// resumes once scrolling settles. Documents without this pattern are unaffected — the
// patch is a passthrough except during the brief guarded window, when nothing calls it.
//
